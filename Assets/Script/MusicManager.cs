...
public class MusicManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        toggle = TransformHelper.GetChild(transform, nameof(toggle)).GetComponent<Toggle>();
   ...
   public void OnToggleChange(bool isOn)
    {
        if (isOn)
        {
            foreach (AudioSource aa in soundAudioS)
            {
                aa.enabled = true;
                aa.Play();
            }
       
           // musicAudioS.enabled = true;
            musicAudioS.Play();
